...
dancells <- WhichCells(object = tissue, expression = Th >= 1 | Slc18a2 >= 1 | Slc6a3 >= 1, slot = "counts")

#Export cells with desired marker expression
dancells <- as.data.frame(dancells)
colnames(dancells) <- "cell_id"
dancells$group <- "InsertTissueIDHere"
write.csv(dancells, "step3_output_selectedCells/dancells.csv")
